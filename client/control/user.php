...
class usercontrol extends base {
	...
	function onget_user($arr) {
		@extract($arr, EXTR_SKIP);//username
		if($status = $_ENV['user']->get_user_by_username($username)) {
	...
	function ondelete($arr) {
		...
		return $_ENV['user']->delete_user(explode(',', $uid));
	}

	function ongetprotected() {
		$this->db->fetch_all("SELECT username FROM ".UC_DBTABLEPRE."protectedmembers GROUP BY username", $protectedmembers);
	...
	function onaddprotected($arr) {
		...
		foreach($usernames as $username) {
			$user = $_ENV['user']->get_user_by_username($username);
			$uid = $user['uid'];
			$this->db->query("INSERT INTO ".UC_DBTABLEPRE."protectedmembers SET uid='$uid', username='$username', appid='$appid', dateline='{$this->time}', admin='$admin'", 'SILENT');
		}
