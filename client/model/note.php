...
class notemodel {
	...
	function sendone($appid, $noteid = 0, $note = '') {
		...
		$getcontent = trim(uc_fopen($url, 500000, $note['postdata'], '', 1, $app['ip'], UC_NOTE_TIMEOUT));

		$returnsucceed = $getcontent != '' && $getcontent != '-1' && $getcontent != '-2';
		if($returnsucceed) {
			$note['app'.$appid] = 1;
		}
	...
	function _get_note() {
		$this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."notelist WHERE closed='0' ORDER BY pri DESC, noteid ASC LIMIT 1", $data);
		return $data;
	}

	function _gc() {
		rand(0, UC_NOTE_GC) == 0 && $this->db->query("DELETE FROM ".UC_DBTABLEPRE."notelist WHERE close='1'");
	}
