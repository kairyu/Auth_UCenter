...
class tagmodel {
	...
	function get_tag_by_name($tagname) {
		$this->db->fetch_all("SELECT * FROM ".UC_DBTABLEPRE."tags WHERE tagname='$tagname'", $arr);
		return $arr;
	}

	function get_template($appid) {
		$this->db->result_first("SELECT tagtemplates FROM ".UC_DBTABLEPRE."applications WHERE appid='$appid'", $result);
		return $result;
	}
	...
	function updatedata($appid, $data) {
		...
		if(!empty($data[0])) {
			$this->db->result_first("SELECT count(*) FROM ".UC_DBTABLEPRE."tags WHERE tagname='$data[0]' AND appid='$appid'", $return);
			if($return) {
	...
	function formatcache($appid, $tagname) {
		$this->db->result_first("SELECT count(*) FROM ".UC_DBTABLEPRE."tags WHERE tagname='$tagname' AND appid='$appid'", $return);
		if($return) {
