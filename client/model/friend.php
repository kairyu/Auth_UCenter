...
class friendmodel {
	...
	function add($uid, $friendid, $comment='') {
		$this->db->result_first("SELECT direction FROM ".UC_DBTABLEPRE."friends WHERE uid='$friendid' AND friendid='$uid' LIMIT 1", $direction);
		if($direction == 1) {
	...
	function get_totalnum_by_uid($uid, $direction = 0) {
		...
		$this->db->result_first("SELECT COUNT(*) FROM ".UC_DBTABLEPRE."friends WHERE $sqladd", $totalnum);
		return $totalnum;
	}
	...
	function get_list($uid, $page, $pagesize, $totalnum, $direction = 0) {
		...
		if($direction == 0) {
			...
			$sqladd = "f.friendid='$uid'";
		}
		if($sqladd) {
			$this->db->fetch_all("SELECT f.*, m.username FROM ".UC_DBTABLEPRE."friends f LEFT JOIN ".UC_DBTABLEPRE."members m ON f.friendid=m.uid WHERE $sqladd LIMIT $start, $pagesize", $data);
			return $data;
		} else {
			return array();
		}
