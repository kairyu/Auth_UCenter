...
class miscmodel {
	...
	function get_apps($col = '*', $where = '') {
		$this->db->fetch_all("SELECT $col FROM ".UC_DBTABLEPRE."applications".($where ? ' WHERE '.$where : ''), $arr);
		return $arr;
	}
	...
	function dfopen($url, $limit = 500000, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE) {
	}
