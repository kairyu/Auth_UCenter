...
class base {
	...
	function get_setting($k = array(), $decode = FALSE) {
		...
		$this->db->fetch_all("SELECT * FROM ".UC_DBTABLEPRE."settings $sqladd", $settings);
	...
	function note_exists() {
		$this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."vars WHERE name='noteexists'", $noteinfo);
		if(!empty($noteinfo) && $noteinfo['noteexists'] == '0') {
			return FALSE;
		} else {
			return TRUE;
		}
