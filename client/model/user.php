...
class usermodel {
	...
	function get_user_by_uid($uid) {
		$this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE uid='$uid'", $arr);
		return $arr;
	}

	function get_user_by_username($username) {
		$this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE username='$username'", $arr);
		return $arr;
	}
	...
	function check_mergeuser($username) {
		$this->db->result_first("SELECT count(*) FROM ".UC_DBTABLEPRE."mergemembers WHERE appid='".UC_APPID."' AND username='$username'", $data);
		return $data;
	}
	...
	function check_usernameexists($username) {
		$this->db->result_first("SELECT username FROM ".UC_DBTABLEPRE."members WHERE username='$username'", $data);
		return $data;
	}
	...
	function check_emailexists($email) {
		$this->db->result_first("SELECT email FROM  ".UC_DBTABLEPRE."members WHERE email='$email'", $email);
		return $email;
	}
	...
	function edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw = 0) {
		$this->db->fetch_first("SELECT password, salt FROM ".UC_DBTABLEPRE."members WHERE username='$username'", $data);
	...
	function delete_user($uidsarr) {
		...
		$this->db->query("SELECT uid FROM ".UC_DBTABLEPRE."protectedmembers WHERE uid NOT IN($uids)", $arr);
		$uids = $comma = '';
		foreach($arr as $v) {
			$uids .= $comma."'".$v['uid']."'";
			$comma = ",";
	...
	function get_total_num($sqladd = '') {
     		$this->db->result_first("SELECT COUNT(*) FROM ".UC_DBTABLEPRE."members $sqladd", $data);
		return $data;
	}

	function get_list($page, $ppp, $totalnum, $sqladd) {
		$start = $this->base->page_get_start($page, $ppp, $totalnum);
		$this->db->fetch_all("SELECT * FROM ".UC_DBTABLEPRE."members $sqladd LIMIT $start, $ppp", $data);
		return $data;
	}
